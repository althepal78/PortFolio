...
const Header = () => {
  ...
  // Add effect to toggle no-scroll-y class on body
  useEffect(() => {
    if (isMenuOpen) {
  ...
  return (
    <header className="site-header flex justify-between items-center px-6 py-4">
      <Link to="/" className={`site-logo relative ${isMenuOpen ? 'opacity-0' : 'z-10'}`}>
        <img 
          src="/logo.jpg" 
          alt="Logo" 
      ...
      <div 
        className={`mobile-menu fixed top-0 left-0 w-full h-full bg-black flex flex-col justify-center items-center transition-all duration-300 ease-in-out md:hidden z-40 ${
          isMenuOpen ? 'menu-open opacity-100 visible' : 'menu-closed opacity-0 invisible'
        }`}
      >
